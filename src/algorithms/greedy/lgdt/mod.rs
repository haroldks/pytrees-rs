    ...
    D: OptimalDepth2Tree
{
    fn fit(&mut self, cover: &mut Cover ) -> Result<(), FitError>{
        ...
        solution_tree.update_root()
            .map(|updater| updater
                .value(root_tree.root_details()));
        self.recursion(self.config.max_depth - 1, cover, &mut solution_tree, root_index , root_attribute)?;
...
where 
    D: OptimalDepth2Tree
{
    fn recursion(&self, depth: usize, cover: &mut Cover, tree: &mut Tree, parent: usize, attribute: usize) -> Result<f64, FitError>{
        let mut parent_error = 0.0;
        for branch_value in [0, 1] {
            let support = cover.branch_on(item(attribute, branch_value));

            if support < self.config.min_support {
                parent_error += self.create_leaf_node_in_tree(tree, parent, branch_value == 0, cover);
                cover.backtrack();
                continue;
            }

            if depth <= 1 {
                let child_tree_result = self.search.fit(self.config.min_support, depth, cover, None);
                    ...
                    Ok(child_tree) => {
                        ...
                        if float_is_null(child_tree.root_error()) {
                            ...
                            tree.update_node(child_index)
                                .map(|updater| updater
                                    .value(child_tree.root_details()));
                            let next_attribute = child_tree.node_test(child_tree.get_root_index()).ok_or(FitError::AlgorithmError)?;
                            error = self.recursion(depth - 1, cover, tree, child_index, next_attribute);
                        }
    ...
    fn create_leaf_node_in_tree(&self, tree: &mut Tree, parent: usize, left: bool, cover: &mut Cover) -> f64 {
