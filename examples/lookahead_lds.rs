...
use dtrees_rs::parsers::examples::{
    load_or_create_result, make_discrepancy_rule, run_iterative, ExampleParser,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let method = match app.cache_type {
        CacheType::Trie => "trieanytimesplit-lds",
        CacheType::Hashmap => "hashanytimesplit-lds",
    };
    let checkpoint = 10;
    ...
    let stats = match app.cache_type {
        CacheType::Trie => {
            let mut algo = DL85Builder::default()
                ...
                .add_search_rule(Box::new(make_discrepancy_rule(app.step)))
