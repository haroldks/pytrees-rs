//! Anytime DL8.5 restricted by a `GainRule`, recording the error of every
//! pass as JSON in `<result>/<dataset>/`. Run with `--help` for the options.
...
use dtrees_rs::algorithms::optimal::rules::{Exponential, GainRule, Luby, Monotonic};
