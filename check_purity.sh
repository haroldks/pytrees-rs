...
COMMAND="target/release/examples/purity"
...
for ((depth = START_DEPTH; depth <= END_DEPTH; depth += STEP)); do
    echo "-------------------------------------------"
    echo "Executing: $COMMAND --input $INPUT_FILE --depth $depth --support 1 --timeout 300 --metric 0.0 --epsilon 0.002 --result . $@"
    echo "-------------------------------------------"

    $COMMAND --input "$INPUT_FILE" --depth "$depth" --support 1 --timeout 300 --metric 0.0 --epsilon 0.002 --result . "$@"
